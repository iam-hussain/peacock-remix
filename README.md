...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create peacock
  fly apps create peacock-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app peacock
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app peacock-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'peacock-staging' does not match app name in config file 'peacock'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name peacock-db
  fly postgres attach --app peacock peacock-db

  fly postgres create --name peacock-staging-db
  fly postgres attach --app peacock-staging peacock-staging-db
  ```

  > **Note:** You'll get the same warning for the same reason when attaching the staging database that you did in the `fly set secret` step above. No worries. Proceed!

Fly will take care of setting the `DATABASE_PRISMA_URL` secret for you.
